...
export default function SignUpPage() {
  ...
  const [nicknamePhone, setNicknamePhone] = useState('');
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    const { user, error: authError } = await auth.signUp(email, password, nickname);

    if (authError || !user) {
      setError(authError?.message || t('auth.error.signupFailed'));
      setLoading(false);
      return;
    }
        ...
        {authMethod === 'phone' && (
          <form className="mt-8 space-y-6" onSubmit={handlePhoneSubmit}>
            <div className="space-y-4">
              <div>
                <label htmlFor="nicknamePhone" className="block text-sm font-medium text-gray-700 mb-1">
                  {t('auth.nickname')}
                </label>
                <input
                  id="nicknamePhone"
                  ...
                  placeholder={t('auth.nickname')}
                  value={nicknamePhone}
                  onChange={(e) => setNicknamePhone(e.target.value)}
                />
              </div>
              <div>
